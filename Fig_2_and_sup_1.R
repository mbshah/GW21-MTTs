...
# Read the Excel file
file_path <- "/home/hb0358/Disk2/Disk2/Data/MTT_mbs/Statistics_GW21/SNR_Analysis_2_sig.xlsx"
...
bar_plot2 <- ggplot(long_data, aes(x = as.factor(Time), y = mean, fill = Pair_n)) +
  ...
  labs(x = "Time (Hours)", y = "Bray-Curtis Dissimilarity", fill = "Pair") +theme_minimal()+
  theme(
...
line_plot3 <- ggplot(long_data, aes(x = Time, y = mean, fill = Pair_n, group = interaction(Measure, Pair_n))) +
  ...
  labs(x = "Time (Hours)", y = "Signal to Noise Ratios") +
  theme_minimal() +
